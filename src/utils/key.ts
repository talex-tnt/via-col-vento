...
export function getTitleForByte(
  byte: number,
  size = 100,
  basicKeyToByte: Record<string, number>,
  byteToKey: Record<number, string>,
  ...
  return basicKeycode?.title ??  basicKeycode?.name ??getShortNameForKeycode(basicKeycode, size);
}
...
function getBasicKeys(): IKeycodeMenu {
  return {
    id: 'basic',
    label: 'Basic',
    keycodes: [
...
function getSpecialKeys(): IKeycodeMenu {
  return  {
    id: 'special',
    label: 'Special',
    width: 'label',
